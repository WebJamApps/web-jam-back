...
const findUserById = async (req: { user: any; userType: string; baseUrl:string }, 
  ...
  debug(req.userType);
  debug(req.baseUrl);
  const authRoles:any = JSON.parse(process.env.AUTH_ROLES || '{}');
  const route = req.baseUrl.split('/')[1];
  // eslint-disable-next-line security/detect-object-injection
  const rolesArr = authRoles[route];
  if (rolesArr.indexOf(req.userType) === -1) return res.status(401).json({ message: 'The user does not have the permission' });
  return next();
};
class AuthUtils {
  static createJWT(user: {_id:string}): string {
    const payload = {
      sub: user._id,
      iat: moment().unix(),
  ...
  static ensureAuthenticated(req: any, res: any, next: any): Promise<any> {
    if (!req.headers.authorization && !req.headers.Authorization) {
      return res.status(401).send({ message: 'The request does not have an Authorization header' });
    }
    let payload = { sub: '' }, token:string = req.headers.authorization || req.headers.Authorization;
