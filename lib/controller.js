...
class Controller {
  ...
  findById(req, res) {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
      return res.status(400).send({ message: 'Find id is invalid' });
    }
  ...
  findByIdAndUpdate(req, res) {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) return res.status(400).send({ message: 'Update id is invalid' });
  ...
  findByIdAndRemove(req, res) {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) return res.status(400).send({ message: 'id is invalid' });
    return this.model.findByIdAndRemove(req.params.id)
      .then((doc) => {
        if (!doc) return res.status(400).send({ message: 'Delete id is invalid' });
